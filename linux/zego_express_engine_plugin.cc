...
static void zego_express_engine_plugin_handle_method_call(
    ZegoExpressEnginePlugin* self,
  ...
  if (pair != G_MethodMap.end()) {
      try {
          if (!pair->second.second &&
              ...
              fl_method_call_respond_error(method_call, "Engine_not_created", "Please call createEngineWithProfile first", nullptr, nullptr);
          } else {
              pair->second.first(argument, std::move(result));
          }
      } catch (std::exception &e) {
          fl_method_call_respond_error(method_call, "method_call_error", e.what(), nullptr, nullptr);
      }
  } else {
...
static FlMethodErrorResponse* listen_cb(FlEventChannel* channel, FlValue *args, gpointer user_data) {
  send_events = TRUE;
  return NULL;
}

static FlMethodErrorResponse* cancel_cb(FlEventChannel* channel, FlValue *args, gpointer user_data) {
  send_events = FALSE;
  return NULL;
}

void zego_express_engine_plugin_register_with_registrar(FlPluginRegistrar* registrar) {
  ZegoExpressEnginePlugin* plugin = ZEGO_EXPRESS_ENGINE_PLUGIN(
      g_object_new(zego_express_engine_plugin_get_type(), nullptr));
  ...
  fl_method_channel_set_method_call_handler(method_channel, method_call_cb,
                                            g_object_ref(plugin),
                                            g_object_unref);
  FlEventChannel* event_channel = fl_event_channel_new(messenger, "plugins.zego.im/zego_express_event_handler", FL_METHOD_CODEC (codec));
