...
void ZegoTextureRenderer::CreateTexture(flutter::TextureRegistrar *texture_registrar,
...
ZegoTextureRenderer::ConvertPixelBufferForFlutter(size_t target_width, size_t target_height) {
    ...
    if (/*data_size > 0 &&*/ srcBuffer_.size() > 0 /*== data_size*/) {
        ...
        if (!flutterDesktopPixelBuffer_) {
            flutterDesktopPixelBuffer_ = std::make_unique<FlutterDesktopPixelBuffer>();

            // Unlocks mutex after texture is processed.
            flutterDesktopPixelBuffer_->release_callback = [](void *release_context) {
